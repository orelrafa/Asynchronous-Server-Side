...
router.post('/', async (req, res) => {
    ...
    try {
      ...
      if(!(["food","health","housing","sport","education","transportation","other"].includes(category))){
        return res.status(400).json({ error: 'Invalid category entered or missing'})
      }
  ...
  //Function to check if entered date is valid. for example if we entered month=2 day=30 then it's invalid.
  function isDateValid(month,day,year){
    // When we pass 0 in the day parameter, we get the last day of a specific month
    const lastDayOfMonth = new Date(year, month, 0);

    // With getDate() we get day. for example if the date is 28/02/2024 we get 28
